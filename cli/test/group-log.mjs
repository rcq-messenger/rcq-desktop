...
// contiguous-prefix rule, and the live-frame ack. No socket, no network.
//
// Run: npm run cli:test   (builds first; this imports the BUILT bundle)

import assert from 'node:assert/strict'
import { ackLiveGroupRow, drainGroupLog, forgetVouched, vouchedSeq } from '../dist/group-v1.mjs'
...
// back by it. The next drain re-serves from the hole.
{
  ...
  assert.equal(vouchedSeq(API, UIN, 21), 1)
  flaky = false
  got.length = 0
...
console.log('group log ok: head start, page loop, contiguous prefix, no spin, ack failure, malformed rows, live ack')
